<?php
	...
	include_once ("../config/config.php");
		if(isset($_GET['cat_id'])){
			$sqlEditCate = "SELECT * FROM categories WHERE cat_id =".$_GET['cat_id'];
		...
		if(isset($_POST ['sbm'])){
			if(empty($_POST ['cat_name'])){
				...
				$sqlCheck = "SELECT * FROM categories WHERE cat_name = '$cat_name'";
				$queryCheck = mysqli_query ($conn, $sqlCheck);
				if(mysqli_num_rows($queryCheck) > 0){ 
					$errors ['error'] = '<div class="alert alert-danger"> Danh mục đã tồn tại !</div>';
				}else{ 
					$sqlUpdateCate = "UPDATE categories SET cat_name = '$cat_name' WHERE cat_id =" .$_GET['cat_id'];// cbi truy vấn
                ...
                <div class="panel panel-default">
                    <div class="panel-body">
						...
						<?php
                            ...
                            <input type="submit" name="sbm" class="btn btn-primary" value="Cập nhật"></input>
